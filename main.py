...
survey_2017 = pd.read_csv(path + "/data set/OSMI Mental Health in Tech Survey 2017.csv", na_values = np.nan)
survey_2017['year'] = '2017'
survey_2018 = pd.read_csv(path + "/data set/OSMI Mental Health in Tech Survey 2018.csv", na_values = np.nan)
survey_2018['year'] = '2018'
survey_2019 = pd.read_csv(path + "/data set/OSMI Mental Health in Tech Survey 2019.csv", na_values = np.nan)
survey_2019['year'] = '2019'
survey_2020 = pd.read_csv(path + "/data set/OSMI Mental Health in Tech Survey 2020 .csv", na_values = np.nan)
...
Mh_avg_no_samples  = prep_data[prep_data['tech_role_flag'] == 1 ][['year', 'country_work_in', 'curr_have_mh_disorder']]                    .groupby(by = ['year', 'country_work_in'])                    .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'no.samples'})


# In[22]:


Mh_avg  = prep_data[(prep_data['tech_role_flag'] == 1 ) & ( np.isin(prep_data['country_work_in'], country))][['year', 'country_work_in', 'curr_have_mh_disorder']]            .groupby(by = ['year', 'country_work_in', 'curr_have_mh_disorder' ])            .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'count'})
Mh_avg['Avg'] = 0 
...
def draw_world_cloud():
    ...
    my_wordcloud = WordCloud(stopwords=STOPWORDS,
                            ...
                            collocations=True,
                            # contour_width=5, 
                            mask=np.array(Image.open(path + "/assets/b2.jpg"))).generate(words)
...
def draw_MH_interview (country): 
   ...
   ############################################## Calculations #################################################
   admit_ph_issue_in_interview  = prep_data[prep_data['tech_role_flag'] == 1 ][['year', 'country_work_in', 'admit_ph_issue_in_interview']]            .groupby(by = ['year', 'country_work_in'])            .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'no.samples'})
   admit_mh_issue_in_interview  = prep_data[prep_data['tech_role_flag'] == 1 ][['year', 'country_work_in', 'admit_mh_issue_in_interview']]               .groupby(by = ['year', 'country_work_in'])               .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'no.samples'})
   ...
   ## build my dataframe
   admit_ph_issue_in_interview_avg  = prep_data[(prep_data['tech_role_flag'] == 1 ) & ( np.isin(prep_data['country_work_in'], country))][['year', 'country_work_in', 'admit_ph_issue_in_interview']]               .groupby(by = ['year', 'country_work_in', 'admit_ph_issue_in_interview' ])               .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'count'})
   ...
   #### Get the avarage of each country by its own (yes, no, maybe) / no.samples each years to avoid imbalance 
   ## build my dataframe
   admit_mh_issue_in_interview_avg  = prep_data[(prep_data['tech_role_flag'] == 1 ) & ( np.isin(prep_data['country_work_in'], country))][['year', 'country_work_in', 'admit_mh_issue_in_interview']]               .groupby(by = ['year', 'country_work_in', 'admit_mh_issue_in_interview' ])               .size().reset_index().sort_values(by=['year','country_work_in']).rename(columns = {0 : 'count'})
...
#                                                dbc.ButtonGroup(
#                                                     [
#                                                      dbc.Button("Physical Health",color="525252",outline=False,style = {'background-color':'#b3dad7',"border-color":"#525252"},id='b1'), 
#                                                      dbc.Button("Mental Health",color="525252" ,outline=False, style = {'background-color':'#b3dad7',"border-color":"#525252"},id='b2'),
#                                                     ],
#                                                             size="md",className="me-1",id ='btn'),
                                    dbc.RadioItems(
                                                id="radios",
...
#                                     dbc.Tabs([
#                                         dbc.Tab(label="Physical Health", tab_id="tab-1",tabClassName="border-top-0 font-weight-normal",tab_style = {"border-color":"#525252",},label_style={"color": "#525252",'font-family':'Calibri',' font-size': '2.5em'}),
#                                         dbc.Tab(label="Mental Health", tab_id="tab-2",tabClassName="border-top-0 font-weight-normal",tab_style = {"border-color":"#525252",},label_style={"color": "#525252",'font-family':'Calibri',' font-size': '2.5em'}),],id="tabs",
#                                         active_tab="tab-1",className ='bg-white border-top-0 nav-justified',)
